...
class categoryClass:
    ...
    #---------Category Details-----------
        ...
        cat_frame.place(x=50,y=250,width=500,height=300)
        ...
        self.im2 = Image.open("images/cate1.jpg")
        self.im2 = self.im2.resize((500,250))
        self.im2 = ImageTk.PhotoImage(self.im2)

        self.lbl_im2 = Label(self.root,image=self.im2,bd=2,relief=RAISED)
        self.lbl_im2.place(x=700,y=400)
    ...
    def show(self):
        ...
        try:
            ...
            for row in rows:
                self.category_table.insert('',END,values=tuple(row.values()))
            conn.close()
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to: {str(ex)}",parent = self.root)
